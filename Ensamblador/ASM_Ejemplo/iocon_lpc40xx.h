...
enum iocon_configuracion_es{
  ...
  IOCON_REPEATER_MODE = (3u << 3),    //!< Habilitar modo repetidor
  IOCON_HYS = (1u << 5),              //!< Habilitar histéresis
  IOCON_INV = (1u << 6),              //!< Habilitar modo invertido (entrada)
  IOCON_ADMODE = (1u << 7),           //!< Habilitar modo analógico
  IOCON_FILTER = (1u << 8),           //!< Habilitar filtro de glitch
  IOCON_HS = (1u << 8),               //!< Habilitar modo HS para modos estándar del I2C
  IOCON_SLEW = (1u << 9),             //!< Habilitar control del slew rate
  IOCON_HIDRIVE = (1u << 9),          //!< Habilitar HIDRIVE para modo Fast Mode Plus de I2C
  IOCON_OD = (1u << 10),              //!< Habilitar modo Open Drain
  IOCON_DACEN = (1u << 16)            //!< Habilitar salida del DAC
};

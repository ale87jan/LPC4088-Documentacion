...
// ===== IOCON - Constantes Privadas =====
/**
 * @brief   Tabla con las funciones posibles de cada uno de los pines.
 * @ingroup IOCON
 * @private
 */
...
void iocon_configurar_pin(LPC_GPIO_TypeDef *gpio_regs, uint32_t mascara_pin,
  ...
  switch ((uint32_t) gpio_regs) {
    case (uint32_t) PUERTO0:
      numero_puerto = 0;
      break;
    case (uint32_t) PUERTO1:
      numero_puerto = 1;
      break;
    case (uint32_t) PUERTO2:
      numero_puerto = 2;
      break;
    case (uint32_t) PUERTO3:
      numero_puerto = 3;
      break;
    case (uint32_t) PUERTO4:
      numero_puerto = 4;
      break;
    case (uint32_t) PUERTO5:
      numero_puerto = 5;
      break;
    default:
  ...
  ASSERT(mascara_pin != 0, "La mascara de seleccion de pin no selecciona ningun pin.");

  for (numero_pin = 0; numero_pin < 32; numero_pin++) {
    if ((mascara_pin & 1) != 0) {
      ASSERT((mascara_pin & 0xFFFFFFFEu) == 0,
             "La mascara de seleccion de pin selecciona mas de un pin.");
      break;
    }
    mascara_pin >>= 1;
  }

  ASSERT((uint32_t) funcion < (uint32_t) RESERVED, "Funcion no valida.");
  ...
  ASSERT(i < IOCON_MAXIMO_FUNCIONES_POR_PIN, "El pin no tiene la funcion indicada.");

  // Los bits ADMODE (bit 7), FILTER (bit 8) y HS (bit 8) actúan invertidos
  configuracion_es ^= IOCON_ADMODE | IOCON_FILTER;

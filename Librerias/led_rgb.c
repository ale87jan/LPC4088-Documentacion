...
void led_rgb_inicializar(void) {
   ...
   * LED_RGB_PERIODO_PWM_US con el valor correspondiente.
   */
  LPC_PWM0->MCR = (1u << 1);
  LPC_PWM0->MR0 = LED_RGB_PERIODO_PWM_US;
  ...
  /* Ajusta el valor del registro PCR del módulo PWM para habilitar la generación de señales PWM
   * por los canales 2, 4 y 6 del módulo. Debes configurar el modo PWM de control de un solo flanco.
   */
...
bool_t led_rgb_ajustar_color(uint32_t color) {
  ...
  /* Descomponer el color en sus componentes r, g y b teniendo en cuenta que el formato es
   * 8r:8g:8b. Los 8bits más significativos deben estar a 0, en caso contrario se retornará FALSE.
   */
  if ((color >> 24) != 0) {
    return FALSE;
  }
  ...
  LPC_PWM0->MR2 = r * LED_RGB_PERIODO_PWM_US / 255;
  LPC_PWM0->MR4 = g * LED_RGB_PERIODO_PWM_US / 255;
  LPC_PWM0->MR6 = b * LED_RGB_PERIODO_PWM_US / 255;

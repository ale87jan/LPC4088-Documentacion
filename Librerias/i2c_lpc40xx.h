/**
 * @file    i2c_lpc40xx.h
 * @brief   Funciones de manejo de las interfaces I2C del LPC40xx.
 *
 * @author      Alejandro Lara Doña - alejandro.lara@uca.es | Eduardo Romero
 * @date        2014/2025
 * @version     2.0
 *
 * @copyright   GNU General Public License version 3 or later

 * @note    Sólo se han implementado las funciones como maestro.
 */

#ifndef I2C_LPC40XX_H
#define I2C_LPC40XX_H

#include <LPC407x_8x_177x_8x.h>
#include "tipos.h"

/**
 * @defgroup  I2C_Macros_Publicas I2C - Macros Públicas
 * @ingroup   I2C
 * @brief Símbolos alternativos de las interfaces I2C.
 * @{
 */
#define I2C0  LPC_I2C0
#define I2C1  LPC_I2C1
#define I2C2  LPC_I2C2
//!@}

// ===== I2C - Constantes Publicas =====
/**
 * @brief   Constantes para los bits de los registros CONSET y CONCLR.
 * @ingroup I2C
 */
enum i2c_con_bits {
  I2C_CON_AA   = (1u << 2),
  I2C_CON_SI   = (1u << 3),
  I2C_CON_STO  = (1u << 4),
  I2C_CON_STA  = (1u << 5),
  I2C_CON_I2EN = (1u << 6)
};

/**
 * @brief   Constantes para el bit de lectura/escritura.
 * @ingroup I2C
 */
enum i2c_bit_rw {
  I2C_BIT_ESCRIBIR = 0u,
  I2C_BIT_LEER = 1u
};

// ===== I2C - Funciones Publicas =====
void i2c_inicializar(LPC_I2C_TypeDef *i2c_regs, uint32_t frecuencia_scl,
                     LPC_GPIO_TypeDef *puerto_sda, uint32_t mascara_pin_sda,
                     LPC_GPIO_TypeDef *puerto_scl, uint32_t mascara_pin_scl);
void i2c_start(LPC_I2C_TypeDef *i2c_regs);
void i2c_stop(LPC_I2C_TypeDef *i2c_regs);
bool_t i2c_transmitir_byte(LPC_I2C_TypeDef *i2c_regs, uint8_t byte);
uint8_t i2c_recibir_byte(LPC_I2C_TypeDef *i2c_regs, bool_t ack);

#endif  // I2C_LPC40XX_H
